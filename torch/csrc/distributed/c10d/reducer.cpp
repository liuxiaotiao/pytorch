...
void Reducer::all_reduce_bucket(Bucket& bucket) {
  ...
  /*Compute super block = segmentBytes/4*/
  ...
  */
  ...
  // for (size_t i = 0; i < bucket.bucket_views_in.size(); ++i) {
  //     at::Tensor view_flat = bucket.bucket_views_in[i].reshape({-1});
  //     int64_t n_elem = bucket.lengths[i];
  //     // int64_t n_elem = view_flat.size(0);

  //     at::Tensor global_indices;
  //     if (n_elem <= 4096) {
  //       at::Tensor indices = at::arange(n_elem, view_flat.options().dtype(at::kFloat)); // On CUDA
  //       global_indices = indices.add(static_cast<int64_t>(bucket.offsets[i]));
  //     } else {
  //       int topk = view_flat.numel()
  //       auto topk_result = view_flat.abs().topk(k);
  //       at::Tensor indices = std::get<1>(topk_result);
  //       global_indices = indices.add(static_cast<int64_t>(bucket.offsets[i]));
  //     }
  //     all_global_indices.push_back(global_indices);
  // }

  // at::Tensor concat_indices = at::cat(all_global_indices);

  // at::Tensor bucketed_indices = at::div(concat_indices, /*other=*/360, /*rounding_mode=*/"trunc")*360;
  // auto unique_bucketed = at::_unique(bucketed_indices, /*sorted=*/true, /*return_inverse=*/false);
  // // auto unique_indices = std::get<0>(unique_bucketed);

  // bucket.sparse_tensor_indices = std::get<0>(unique_bucketed);
